 ...
 * (i.e. sensor) events are being subscribed to. Upon events being received,
...
export class EventStore implements IEventPublisher, IMessageSource {
  ...
  async publish<T extends IEvent>(event: T) {
    ...
    const sensorId = message.sensorId || message.sensorDto.sensorId;
    const streamName = `${this.category}-${sensorId}`;
